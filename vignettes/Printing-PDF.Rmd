...
To save tables and charts to an Excel file, initiate Excel printing with `set_opts( output = "Excel" )`. Then, use the various tabulation functions to "print" / generate the tables and charts. Finally, call `save_excel()` to save all of these tables and charts to a single Excel file, which consists of one or more spreadsheets, one for each table or chart.

Initiate Excel printing:
...
Save the tables to a workbook:

```r
save_excel(file = "my_workbook.xlsx")
```
...
Before using Excel printing, be sure to install these packages: `openxlsx2` and `mschart`. Finally, to turn off Excel printing, just switch to another output method:
