...
Some analysts might wish to compare the output from `surveytable` to the output from other statistical software, such as SAS / SUDAAN. In this situation, `set_opts(output = "screen", raw = TRUE)` might be useful. This command tells `surveytable` to print unformatted and unrounded tables to the screen.
...
To restructure tables to make them easier to process programmatically, see `restructure()`. Also see `set_opts(output = "screen", raw = TRUE)`.
