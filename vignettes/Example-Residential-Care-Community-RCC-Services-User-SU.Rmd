...
In the published figure, the Hispanic and Other categories have been merged into a single category called "Another race or ethnicity". Let's see how we can do that.  

Each survey design object has an element called `variables`, which is a data frame that contains the survey variables. **We can add variables to this data frame or change the variables currently there.** To collapse the levels of `raceeth2`, we'll use the `forcats::fct_collapse` function.

```{r}
mysurvey$variables$raceeth2 = forcats::fct_collapse(mysurvey$variables$raceeth2
                           , `Another race or ethnicity` = c("Hispanic", "Other"))
```
...
`age2` is a numeric variable. We need to create a categorical variable based on this numeric variable. This is done using the `cut` function. 

```{r}
mysurvey$variables$Age = cut(mysurvey$variables$age2
                             , breaks = c(-Inf, 64, 74, 84, Inf)
                             , labels = c("Under 65", "65-74", "75-84", "85 and over"))
```
...
Now, count how many chronic conditions were present.
...
For generating the figure, create a categorical variable based on `num_cc`, which is numeric. When using `cut`, to avoid errors, it is a good practice for `breaks` to start at `-Inf` and end at `Inf`.

```{r, results='asis'}
mysurvey$variables$`Number of chronic conditions` = cut(mysurvey$variables$num_cc
                                            , breaks = c(-Inf, 0, 1, 3, 10, Inf)
...
For generating the figure, create a categorical variable based on `num_adl`, which is numeric. When using `cut`, to avoid errors, it is a good practice for `breaks` to start at `-Inf` and end at `Inf`.

```{r, results='asis'}
mysurvey$variables$`Number of ADLs` = cut(mysurvey$variables$num_adl
                                          , breaks = c(-Inf, 0, 2, 6, Inf)
                                          , labels = c("0", "1-2", "3-6", "??"))
tab("Number of ADLs")
```
